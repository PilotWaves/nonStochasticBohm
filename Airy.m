...
for n = 1:N_half-1
    ...
    vy(n+1) = C*(vy(n) + K*cos(2*pi*a(n+1))*sin(2*pi*sqrt(y(n)^2 + z(n)^2))*exp(-nu*sqrt(vy(n)^2 + vz(n)^2)));
    y(n+1) = y(n) + vy(n+1);
    vz(n+1) = C*(vz(n) + K*cos(2*pi*a(n+1))*cos(2*pi*sqrt(y(n)^2 + z(n)^2))*exp(-nu*sqrt(vy(n)^2 + vz(n)^2)));
    z(n+1) = z(n) + vz(n+1);

end

    if y(N_half)^2 + z(N_half)^2 < 100
        ...
        z(N_half + 1) = 2*z(N_half) - z(N_half-1);
        b(N_half + 1) = b(N_half);
        a(N_half + 1) = a(N_half);
        vy(N_half + 1) = vy(N_half - 1);
        vz(N_half + 1) = vz(N_half - 1);
        ...
        for n = N_half:N-1
            ...
            vy(n+1) = C*(vy(n) + K*cos(2*pi*a(n+1))*sin(2*pi*sqrt((y(n)-y(N_half))^2 + (z(n)-z(N_half))^2))*exp(-nu*sqrt(vy(n)^2 + vz(n)^2)));
            y(n+1) = y(n) + vy(n+1);
            vz(n+1) = C*(vz(n) + K*cos(2*pi*a(n+1))*cos(2*pi*sqrt((y(n)-y(N_half))^2 + (z(n)-z(N_half))^2))*exp(-nu*sqrt(vy(n)^2 + vz(n)^2)));
            z(n+1) = z(n) + vz(n+1);
        end
